...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        length = len(legal)
        particledistribution = self.numParticles / length
        self.particleList = []
        for l in legal:
            for i in range(particledistribution):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance != None:
            for positon in self.legalPositions:
                truedistance = util.manhattanDistance(positon, pacmanPosition)
                allpossible[positon] = emissionModel[truedistance] * belief[positon]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(self.particleList)):
            newPosDist[self.particleList[i]] = self.getPositionDistribution(
                self.setGhostPosition(gameState, self.particleList[i]))
            self.particleList[i] = util.sample(newPosDist[self.particleList[i]])
        return
        util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newPosDist = getPositionDistributionForGhost \
                    (setGhostPositions(gameState, oldParticle), i, agent)
