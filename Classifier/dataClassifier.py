...
def enhancedFeatureExtractorDigit(datum):
    ...
    Number_of_Zero=0
    half_datum_width=DIGIT_DATUM_WIDTH / 2
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT-1):
            if datum.getPixel(x,y)==0:
                Number_of_Zero=Number_of_Zero+1
        if Number_of_Zero >  half_datum_width:
            features[x] = 0
        else:
            features[x] = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    newGhostpos= successor.getGhostPositions()
    min_ghost=float("inf")
    for ghost in newGhostpos:
        val=util.manhattanDistance(newPos,ghost)
        if val<min_ghost:
            min_ghost=val
    ...
    features["nearestghost"] = min_ghost

    food_dist=[]
    foods=successor.getFood().asList()
    min_val=float("inf")
    for food in foods:
        val=util.manhattanDistance(newPos,food)
        if val<min_val:
            min_val=val


    features["food_dist"]=1.0/min_val
